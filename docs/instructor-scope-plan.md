...
앱에서 `clients` 로 가는 쓰기는 넷뿐이고 전부 안전하다.

| 경로 | 방식 | 기존 문서를 덮나 |
| --- | --- | --- |
| 등록·수정·보관 (dual-write) | `setDoc(…, { merge: true })` | 아니오 |
| 등록 화면 `createClient` | merge 없는 `setDoc` | 문서 id 가 매번 새 UUID — 기존 문서를 가리킬 수 없다 |
| 엑셀 이관 | 배치 write | `exists` 로 이미 있으면 건너뛴다 |
| 삭제 | `deleteDoc` | 해당 없음 |

그래서 `instructorIds` 가 **서버에서 지워지지 않고**, `immutable` 규칙이 57 의
회원 저장을 **거부하지도 않는다** — 안 보낸 필드는 `affectedKeys()` 에 들어가지
않는다. 규칙 테스트가 이것을 고정한다.

**그래서 이 규칙은 ①에 같이 나간다.** 잠그기만 하는 규칙이라 읽기를 좁히지
않는다 — ③ 의 `array-contains` 와는 다른 건이다.

### 야간 재계산

`rebuildInstructorIdsNightly` — 매일 **04:00 KST**. 수업이 없고 자정 직후의
만료가 이미 지나간 시각이다. 바뀐 것이 없으면 아무것도 쓰지 않으므로 평소에는
읽기만 하고 끝난다 — 쓰기가 없으면 `memberViews` 트리거도 깨어나지 않는다.

한 센터가 실패해도 나머지는 돈다.

### 대표 화면 — 담당 강사

더보기 → 센터 운영 → **담당 강사**. 감사 로그·번호 점검 옆이다.

- **점검**: 전체 회원 / 담당 있음 / **운영중 · 담당 없음**, 그리고 강사별 담당 회원 수
- **재계산**: 미리보기(`dryRun`) → 몇 명이 바뀌는지 확인 → 실행

미리보기가 먼저인 이유는 **되돌리는 문이 없기 때문이다.** 눌렀는데 백 명이
바뀌면 그 백 명이 맞는지 확인할 방법이 그 자리에 없다.

세 상태를 가른다 — 정상 / 빠진 회원 있음 / **아직 한 번도 안 돌았음**. 셋 다
재계산으로 끝나지만 대표가 읽는 뜻이 다르다. 처음 켠 날과 트리거가 실패한 날은
다른 일이다.
...
② 2단계가 담긴 빌드 출시 → 강사 전원이 그 번호 이상인지 확인
③ 규칙 배포                          ← 대표가 지시할 때만
```

### 60 은 2단계가 아니다

**빌드 60(1.1.29)은 이미 Play 내부 테스트에 올라가 있다 (2026-09-26 게시).**
커밋은 `5be765b` 이고, 59 대비 들어간 것은 **시계 의존 고침 하나뿐**이다
(`deductPass` 의 "지금" 이 두 개였던 버그 포함, 3파일 24줄).

60 에 **없는 것**: 사진 크기 고침(`7d5c843`), 0단계(`71e7110`),
1단계(`d7692f6`), 야간 재계산과 대표 화면(`3dcc1ef`).

그러므로 **③ 의 기준은 60 이 아니다.** 2단계(쿼리를 `array-contains` 로 바꾸는
커밋)가 담긴 빌드 번호이고, 그것은 **61 이상**이 된다. 앱 버전 기록 화면도 그
번호로 판정한다.

### 어느 커밋으로 만든 빌드인지 남긴다

빌드 번호만으로는 "그 번호에 무엇이 들어 있었나" 를 나중에 알 수 없다. 릴리스
커밋은 번호만 올리므로 git 로그를 봐도 번호와 기능이 이어지지 않는다 — 실제로
"60 에 사진 고침이 들어갔나" 를 되짚어야 했다.

장치는 이미 있었다. `VITE_BUILD_COMMIT` → `RELEASE_COMMIT_SHORT` → 더보기의
버전 줄. iOS 쪽 `prepare_build_metadata.py` 만 채우고 **안드로이드 빌드는 비어
있었다.** `tools/android/build-aab.mjs` 가 같은 이름으로 심도록 이었다.

다음 AAB 부터 화면에 `1.1.30 (61) · a1b2c3d` 로 보인다.

### 57 · 59 · **60** 이 ③ 이후 어떻게 깨지나 — **전체 실패다**
