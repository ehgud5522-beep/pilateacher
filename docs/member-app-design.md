...
1. 강사 앱 → 회원 등록: 이름 `테스트(대표)`, 지점 **반송점**, 연락처 대표 번호
2. 회원 앱에서 그 번호로 인증 → `linked` 로 홈이 열리는지
3. 회원권 없음 → "첫 수업을 기다리고 있어요"
4. 회원권 1건 발급 → 트리거가 돌고 → 새로고침 → 잔여가 맞는지
5. 차감 1건 → 이력 한 줄, 잔여 -1
6. 대표 화면 → 회원 앱 → **끊기** → 투영도 지워지는지
