...
/* 비포 사진의 윤곽선만 뽑아 애프터 위에 얹는다 — 겹쳐도 눈이 안 아픈 비교 */
function EdgeLayer({ photo, color }) {
  const ref = useRef(null);
  const key = `${photo?.src || ""}|${photo?.x || 0}|${photo?.y || 0}|${photo?.scale || 1}|${photo?.rot || 0}|${color}`;
  useEffect(() => {
    let alive = true;
    const c = ref.current;
    ...
    img.onload = () => {
      ...
      try {
        ...
        coverDraw(oc, img, W, H, photo);
        const src = oc.getImageData(0, 0, W, H).data;
        const g = new Float32Array(W * H);
        for (let i = 0; i < W * H; i++) g[i] = src[i * 4] * 0.299 + src[i * 4 + 1] * 0.587 + src[i * 4 + 2] * 0.114;
        const ctx = c.getContext("2d");
        ...
        for (let y = 1; y < H - 1; y++) {
          for (let x = 1; x < W - 1; x++) {
            ...
            if (mag > 85) { const o = i * 4; out.data[o] = R; out.data[o + 1] = G; out.data[o + 2] = B; out.data[o + 3] = Math.min(255, mag * 1.4); }
          }
        }
          ...
          {before && after ? (
            <>
              <div className="mb-2 flex gap-1 rounded-2xl p-1" style={{ backgroundColor: CANVAS }}>
                {[{ k: "curtain", l: "커튼" }, { k: "flip", l: "꾹 눌러 전환" }, { k: "edge", l: "윤곽선" }].map((o) => (
              ...
              <div ref={boxRef} className="relative select-none overflow-hidden rounded-2xl bg-photo" style={{ aspectRatio: "3 / 4", touchAction: cmp === "curtain" ? "none" : "auto" }}
                onPointerDown={(e) => { if (cmp === "flip") setFlip(true); else if (cmp === "curtain") { dragRef.current = true; e.currentTarget.setPointerCapture?.(e.pointerId); moveCurtain(e); } }}
                onPointerMove={(e) => { if (cmp === "curtain" && dragRef.current) moveCurtain(e); }}
                onPointerUp={() => { setFlip(false); dragRef.current = false; }}
                onPointerCancel={() => { setFlip(false); dragRef.current = false; }}
                onPointerLeave={() => { setFlip(false); dragRef.current = false; }}>
                {cmp === "flip" ? (
                  <>
                    <img src={(flip ? after : before).src} alt={flip ? "애프터" : "비포"} className="absolute inset-0 h-full w-full object-cover" style={{ transform: ptf(flip ? after : before) }} />
                    <MarkLayer marks={(flip ? after : before).marks} hideLabel />
                  </>
                ) : cmp === "edge" ? (
                  <>
                    <img src={after.src} alt="애프터" className="absolute inset-0 h-full w-full object-cover" style={{ transform: ptf(after) }} />
                    <EdgeLayer photo={before} color="#FF4D4D" />
                  ...
                  <>
                    ...
                    <div className="pointer-events-none absolute flex h-9 w-9 items-center justify-center rounded-full" style={{ left: `${t}%`, top: "50%", transform: "translate(-50%,-50%)", backgroundColor: "#fff", boxShadow: "0 2px 10px rgba(0,0,0,.5)" }}>
                      <ArrowUpDown size={16} style={{ color: "#17171F", transform: "rotate(90deg)" }} />
                    </div>
                  </>
                )}
                {guides && <GuideOverlay />}
                <span className="absolute left-3 top-3 rounded-full px-2.5 py-1 text-xs font-extrabold text-white" style={{ backgroundColor: "rgba(0,0,0,0.6)" }}>
                  {cmp === "edge" ? `AFTER ${ymd(after.date)} · 빨간선 = 비포` : cmp === "flip" ? (flip ? `AFTER ${ymd(after.date)}` : `BEFORE ${ymd(before.date)}`) : `BEFORE ${ymd(before.date)}`}
                </span>
                {cmp === "curtain" && (
                  <span className="absolute right-3 top-3 rounded-full px-2.5 py-1 text-xs font-extrabold text-white" style={{ backgroundColor: "rgba(0,0,0,0.6)" }}>AFTER {ymd(after.date)}</span>
                )}
              </div>
              <p className="mt-2 text-center text-xs font-semibold" style={{ color: SUB }}>
                {cmp === "curtain" ? "사진을 좌우로 밀면 경계가 움직입니다 · 양쪽 다 선명하게 보입니다"
                  : cmp === "flip" ? "사진을 꾹 누르면 애프터, 손을 떼면 비포 · 달라진 부분이 움직임으로 보입니다"
                  : "비포의 몸 윤곽만 빨간 선으로 애프터 위에 얹었습니다"}
              </p>
            </>
          ) : (
