...
const backStack = [];
/* 우리가 스스로 부른 history.back() 때문에 생긴 popstate 는 무시해야 한다.
   (개발 모드에서 효과가 두 번 실행되면 이게 없을 때 창이 열리자마자 닫힌다) */
let backSwallow = 0;
function useBackClose(open, close) {
  useEffect(() => {
    ...
    return () => {
      ...
      const i = backStack.indexOf(entry);
      if (i >= 0) backStack.splice(i, 1);
      if (!done) { backSwallow += 1; try { window.history.back(); } catch (e) {} }
    };
  }, [open]);
}
...
function ScheduleForm({ draft, members, onClose, onSubmit, onDelete }) {
  ...
  const isPersonal = !!draft.personal;
  ...
  return (
    <Sheet title={`${draft.personal ? "일정" : "수업"} ${draft.id ? "수정" : "등록"}`} onClose={onClose}>
      <div className="space-y-3">
        {isPersonal && (
