...
export function scopeHealthMessage(tally) {
  ...
  if (health === SCOPE_HEALTH.MISSING) {
    /* **재계산을 권하지 않는다.** 일부가 이미 채워져 있는데 비어 있는 회원은
       회원권이 한 번도 나간 적 없는 사람이다 -- 담당을 정할 근거가 없으므로
       몇 번을 다시 계산해도 그대로다. 실제로 첫 실행에서 106명 중 34명이 이
       경우였고, 그때 화면이 "재계산을 눌러 주세요" 라고 말했다. 아무 소용
       없는 일을 시키는 문구다.

       이름을 적지 않는다. 대표가 볼 화면이지만 이 문구는 진단에도 남는다. */
    return `운영중인 회원 ${count(tally.emptyActive)}명에게 담당 강사가 없습니다.`
      + " 회원권이 한 번도 나간 적 없는 회원입니다 — 강사에게는 보이지 않고 대표·FC매니저만 봅니다.";
  }
