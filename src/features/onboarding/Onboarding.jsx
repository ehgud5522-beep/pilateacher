...
export function PermissionGuide({ statuses = null }) {
  return <div className="min-w-0 space-y-2" data-permission-guide>
    ...
    {PERMISSION_GUIDE_ITEMS.map((item) => <div key={item.key} className="flex min-w-0 items-start gap-3 rounded-xl p-3" style={{ backgroundColor: "var(--card)", border: "1px solid var(--line)" }}><span className="shrink-0 text-xl" aria-hidden="true">{item.icon}</span><span className="min-w-0 flex-1"><span className="block text-sm font-extrabold" style={{ color: "var(--ink)" }}>{item.title}</span><span className="mt-0.5 block text-xs leading-relaxed" style={{ color: "var(--sub)" }}>{item.body}</span></span>{statuses?.[item.key] && <span className="shrink-0 rounded-full px-2 py-1 text-caption font-bold" style={{ backgroundColor: "var(--tint)", color: "var(--brand)" }}>{statuses[item.key]}</span>}</div>)}
    <p className="pt-1 text-xs font-extrabold" style={{ color: "var(--brand)" }}>필수 권한 없음</p><p className="text-caption leading-relaxed" style={{ color: "var(--sub)" }}>권한은 해당 기능을 처음 사용할 때 요청합니다.<br />설정에서 언제든 변경할 수 있습니다.</p>
  </div>;
}
