...
const VOICE_SESSION_EVENT_TYPES = new Set([
  ...
  "camera_preview_start_requested", "camera_preview_started",
...
export function appendVoiceSessionDiagnostic(event, details = {}, storage = globalThis.localStorage, clock = () => new Date()) {
  ...
  const entry = {
    ...
    ...(Number.isFinite(Number(details.recordedSeconds)) ? { recordedSeconds: Math.max(0, Number(details.recordedSeconds)) } : {}),
    ...(Number.isFinite(Number(details.trimStart)) ? { trimStart: Math.max(0, Number(details.trimStart)) } : {}),
    ...(Number.isFinite(Number(details.trimEnd)) ? { trimEnd: Math.max(0, Number(details.trimEnd)) } : {}),
    ...(Number.isFinite(Number(details.maxAmplitude)) ? { maxAmplitude: Math.max(0, Math.min(1, Number(details.maxAmplitude))) } : {}),
