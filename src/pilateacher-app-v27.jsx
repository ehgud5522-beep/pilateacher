...
import {
  Users, Settings as SettingsIcon, Search, ChevronRight, ChevronLeft, Plus, Camera,
  MessageSquare, Check, X, Trash2, ArrowLeft, Target, ClipboardList,
  RotateCcw, Sparkles, Copy, ArrowUpRight, ArrowDownRight, Loader2, Pencil, UserPlus,
  Activity, Ticket, Calendar, Clock, Bell, Download, AlertTriangle, LogOut, Mail, Star,
  Sun, Moon, Smartphone, Move, Crosshair, ChevronDown, ImagePlus, SlidersHorizontal, CalendarDays, ArrowUpDown,
  QrCode, Minus, Upload, Link2, Users2,
} from "lucide-react";

/* ==================================================================
   필라티쳐 (Pilateacher) — 회원 체형 변화 · 재등록 관리
   v20 추가: ① 설정 탭 프로필 사진 (등록 · 촬영 · 삭제, 헤더/로그인 화면 반영)
             ② AI 체형 분석 — 사진 업로드 → 관절 자동 인식 → 각도 수치 표시
   v22 반영: 종합평가 날짜 선택 개선 · 골든타임 '복귀' 문구 정리 · 다크 톤다운
   v27: ① 인바디 결과지 QR → 측정값 자동 입력 (JSON·key=value·링크·한글 라벨 4형식)
        ② 비포애프터 '촬영하기' 옆 중복 '불러오기' 제거
        ③ 종합평가 멘트가 실패해도 지금 기록으로 바로 만들어 줌 (빈 화면 없음)
        ④ '오늘 수업' → '당일 수업'  ⑤ 날짜 아래 '오늘로' 글자 제거(날짜를 누르면 오늘로)
        ⑥ 토요일(점심까지) · 일요일(내일 수업 준비) 인사 + 내일 수업 미리 보기
        ⑦ 오늘 수업 · 완료 · 남은 수업 타일 색 분리   ⑧ 기록 탭 수강권↔회원상태 순서 교체
        ⑨ 기록 탭 회원 정보 접기/펼치기   ⑩ 설정 → 회원 인계(DB 넘기기 · 번호로 받아오기)
        ⑪ 수행 능력 평가에 −/+ 1점 버튼
   v26: ① 오늘 수업은 '출석/노쇼/취소' 3개만 → 고르면 다음 단계가 열림
        ② 스와이프를 직접 DOM 으로 그려 부드럽게 + 안내 한 줄로 축약
        ③ 로고 — P 를 타일 정중앙에 세우고 캐릭터를 P 기준으로 정렬
   v25: ① 상태 배지 중복 제거 · 노코멘트 ② 코멘트 저장 버튼 강조
        ③ 끝난 수업은 왼쪽으로 밀어 아래로 ④ 탭 '오늘'→'일정' ⑤ 24시간 표기
        ⑥ 회원 잔여 횟수 정렬 ⑦ 회원 탭은 항상 목록부터
   v24: ① 듀엣도 회원 드롭다운 ② 촬영하기/불러오기/사진 조정 · 선은 뗄 때 확정
        ③ 오늘 수업 한 곳에서 출석·노쇼·취소 ④ 주간 시간표(월~토 · 08~23시)
        ⑤ 오전/오후 선택 표시 ⑥ 휴지통 잘림 ⑦ 아이패드 대응
   v23: ① 수업 등록 개편 — 개인/듀엣만 회원 선택, 그룹은 기구(리포머·캐딜락·체어·바렐) 선택
        ② 이달 누적 수업(실제 차감 기준)을 오늘 브리핑에 표시, 월 집계 카드는 제거
        ③ AI 체형 분석을 비포애프터 위로 이동, 비포애프터 하단 인바디 수치 제거
================================================================== */


/* ================= 토큰 · 테마 =================
   [v21] 라이트/다크 두 벌.
   캔버스(체형 분석)·SVG 로고·recharts 는 CSS 변수를 읽지 못하므로
   CSS 변수 대신 값 자체를 바꿔 끼우고 화면을 다시 그린다.
   다크에서는 그림자가 보이지 않으므로 카드마다 1px 경계선으로 구역을 나눈다. */
const LIGHT = {
...
let BRAND, TOAST, PHOTO;   // BRAND = 채워진 버튼 바탕(흰 글씨 기준), PRIMARY = 어두운 바탕 위 글자·아이콘
...
/* 계정 · 세션은 공용 키, 회원 데이터는 계정별 키로 분리 저장 */
...
const METRICS = {
  /* 색은 테마가 바뀌면 같이 바뀌어야 하므로 getter 로 둔다 */
...
const EQUIP_TYPES = ["리포머", "캐딜락", "체어", "바렐"];   /* [v23] 그룹 수업 기구 */
...
const FOCUS_PRESETS = ["거북목", "라운드 숄더", "골반 전방경사", "골반 후방경사", "척추 측만", "흉추 후만", "요추 불안정성", "복직근 이개", "산후 회복", "무릎 정렬"];
...
/* [v23] 기구 그룹 — 회원 없이 기구로만 등록하는 그룹 수업 */
...
/* [v9] 최근 한 달(28일) 실제 출석 기준 페이스.
   등록한 지 한 달이 안 되면 그 기간만큼만 계산하고 문구도 바꾼다. */
...
function monthGrid(iso) {
  ...
  const lead = (new Date(first + "T00:00:00").getDay() + 6) % 7; // 월요일 시작
...
function blankMember(staff) {
  return {
    ...
    status: "active", endedAt: "", endedReason: "", endedMemo: "",          // [v6] 상태
    holdFrom: "", holdUntil: "", holdReason: "", payments: [],              // [v7] 홀딩 · 결제 이력
...
/* [v11] 저장된 데이터가 어떤 버전이든 같은 모양으로 맞춰 주는 안전장치 */
...
/* [v21] 다크에서는 그림자가 안 보이므로 1px 경계선으로 카드 영역을 분리한다 */
...
/* [v20] 프로필 사진 — 사진이 있으면 사진, 없으면 이름 첫 글자 */
...
/* [v24] 시간 선택 — 오전/오후를 눌린 상태가 보이는 버튼으로 */
const to12 = (hhmm) => {
...
function TimePick({ value, onChange }) {
  /* [v25] 오전·오후를 누르면 시 목록이 24시간(0~11 / 12~23)으로 바뀝니다 */
...
/* 로고 — 인물이 P에 팔을 얹은 모습 (P + T = 필라티쳐)
   [v26] P를 타일 정중앙(50,50)에 세우고, 캐릭터를 P 기준으로 맞춤
   · P 바깥 x 34.5~65.5 / y 24~76  → 가로·세로 모두 한가운데
   · 캐릭터: 머리끝을 P 윗변에, 발끝을 P 밑변에 맞추고 왼팔을 바울 곡선에 얹음
   · 점선 십자는 타일 정중앙(50)을 지나 P의 중심을 가리킴 */
const Logo = ({ size = 64, radius = 0.24 }) => (
  <svg width={size} height={size} viewBox="0 0 100 100" role="img" aria-label="필라티쳐"
    ...
    {/* 인물 — P에 맞춰 이동. 바탕색 두께로 겹치는 부분을 자연스럽게 분리 */}
...
/* 시간대별 인사 — 같은 시간대라도 날짜에 따라 문구가 바뀝니다 (따뜻하게) */
const GREETS = {
...
/* [v27] 주말은 수업 리듬이 다릅니다
   · 토요일 — 점심까지가 이번 주 마지막 수업. 오전엔 응원, 오후엔 마무리 인사
   · 일요일 — 다음 주를 준비하는 날. 내일 만날 회원을 미리 보게 이끄는 문구 */
...
/* [v17] 자주 쓰는 코멘트 문구 — 탭 한 번으로 기록 시간을 줄입니다 */
...
/* ==================================================================
   [v5 신규] 로그인 / 회원가입
   - 소셜 로그인 버튼(카카오·네이버·구글·애플) + 이메일 가입
   - 자동 로그인 체크 시 세션을 저장해 다음 실행에서 로그인 화면을 건너뜀
   - 실제 서비스에서는 각 SDK(Kakao.Auth 등) 또는 Firebase Auth 호출부만
     handleSocial() 안쪽으로 교체하면 나머지 흐름은 그대로 동작합니다.
================================================================== */
function AuthScreen({ accounts, onLogin, onSignup, onToast }) {
  ...
  const [signup, setSignup] = useState(null); // 소셜 최초 가입 정보 입력
  const [f, setF] = useState({ name: "", email: "", pw: "", center: "", phone: "" });

  const handleSocial = (provider) => {
    // 실제 연동 시: const res = await Kakao.Auth.login() 등으로 프로필을 받아옴
                ...
                <PrimaryBtn
                  ...
                  onClick={() => {
                    if (emailTab === "signup") {
                      ...
                      // 데모용 검증 — 실제 서비스는 서버/Firebase Auth에서 검증해야 합니다
...
function detectAlerts(members, schedule) {
  const out = [];
  members.forEach((m) => {
    if (isEnded(m)) return;                       // 종료 회원 제외
    const att = attendanceOf(schedule, m.id);
    if (isHold(m)) {                              // [v7] 홀딩 → 복귀 예정 알림
    ...
    /* [v8] 한 달 기준 — ①주당 페이스로 4주 내 소진 예상 ②잔여 10회 미만 ③만료 30일 미만 */
    const rest = left(m);
...
/* ==================================================================
   스케줄 · 예약 관리
   [v5 변경] 노쇼 처리 후 '1회 차감 / 차감 없음'을 그 자리에서 선택
================================================================== */
/* ==================================================================
   스케줄 · 예약 관리 (v9)
   - 일간 / 주간 / 월간(달력) 3가지 보기
   - 주간·월간은 좌우 드래그로 부드럽게 이동
   - 그룹 수업: 한 수업에 여러 회원, 회원별로 출석/노쇼 처리
   - 이번 달 수업 수는 달이 바뀌면 자동으로 새로 집계(월별 저장)
================================================================== */
function ScheduleManager({ db, onSave, onDelete, onStatus, onNoshowFee, onGroupDone, onOpenMember, onWriteNote, onNoComment }) {
  ...
  /* [v23] 이달 누적 — 실제로 차감된 수업(출석·노쇼 차감)과 완료 처리한 그룹 수업만 센다 */
  ...
  /* [v15] 오늘 만나는 회원 — 스케줄에서 바로 회원 관리로 이어지는 동선 */
  const T0 = todayISO();
  const [parked, setParked] = useState({});          /* [v25] 왼쪽으로 밀어 아래로 내린 수업 */
  /* [v24] 오늘 수업 한 줄 = 회원 1명(또는 기구 그룹 1건). 같은 회원이 두 번 수업하면 둘 다 나옵니다 */
  ...
  /* [v27] 내일 수업 — 오늘 수업이 없는 날(주로 일요일) 다음 수업을 미리 보게 합니다 */
  const T1 = shift(T0, 1);
  ...
  /* [v25] 끝난 수업(기록까지 마쳤거나 취소·완료)은 목록 맨 아래로 — 다음 수업이 위로 올라옵니다 */
  const isSettled = (r) => {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="rounded-3xl p-5" style={{ background: `linear-gradient(140deg, ${BRAND} 0%, #5B3FD4 100%)`, boxShadow: SHADOW }}>
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            ...
            {/* [v19] 이름은 한 덩어리로 고정, 인사말은 어절 단위로만 줄바꿈 — 좁은 폰에서도 배열이 깨지지 않음 */}
            {db.settings?.staff && (
        ...
        {/* [v27] 세 칸을 한눈에 구별 — 전체(반투명) · 완료(민트) · 남은 수업(흰 카드로 강조) */}
      ...
      {/* [v27] 오늘 수업이 없는 날 — 내일 만날 회원을 미리 봅니다 (일요일 준비 루틴) */}
      ...
      {/* [v27] 당일 수업 — 출석 · 노쇼 · 취소를 여기서 모두 처리합니다 */}
      {todayRows.length > 0 && (
        <Card className="p-5">
            ...
            {sortedRows.map((r) => {
              ...
              return wrap(
                <div className="rounded-2xl p-3" style={{ backgroundColor: CANVAS }}>
                  ...
                  {/* [v26] 1단계 — 아직 안 골랐으면 세 가지만 보여 줍니다 */}
                  {r.status === "booked" ? (
                    <div className="mt-2 flex gap-1.5">
      ...
      <Card className="p-4">
        <div className="flex items-center gap-2">
          ...
          {/* [v27] '오늘로' 글자는 빼고, 날짜를 누르면 오늘로 돌아옵니다 */}
...
/* [v26] 미는 카드 — 끄는 동안에는 DOM 을 직접 움직여 끊김 없이 따라옵니다
   · 허용된 방향으로만 움직이고, 끝에서는 고무줄처럼 저항이 생깁니다
   · 세로로 먼저 움직이면 스크롤에 양보합니다 */
...
function SwipeRow({ children, down, enabled, onPark, onUnpark }) {
  ...
  const move = (e) => {
    ...
    if (s.axis !== "x") return;
    const dir = down ? 1 : -1;                       /* 내린 카드는 오른쪽으로만, 아니면 왼쪽으로만 */
    const v = dx * dir;
...
/* ==================================================================
   [v24] 주간 시간표 — 월~토, 오전 8시부터 오후 11시까지
   한 칸 = 1시간. 수업은 시작 시각·길이에 맞춰 블록으로 얹습니다.
================================================================== */
const GRID_H0 = 8, GRID_H1 = 23, GRID_ROW = 44;   /* 08:00 ~ 23:00 */
const minOf = (hhmm) => Number(String(hhmm || "0:00").slice(0, 2)) * 60 + Number(String(hhmm || "0:00").slice(3, 5) || 0);
const hourLabel = (h) => `${String(h).padStart(2, "0")}시`;    /* [v25] 24시간 표기 */
...
function WeekGrid({ days, byDate, nameOf, cursor, onOpen, onNew }) {
  ...
  const blocksOf = (d) => byDate(d).filter((s) => {
    /* 표에 그릴 수 있는 시간대(08:00~24:00)와 겹치는 수업만 */
...
function ScheduleForm({ draft, members, onClose, onSubmit, onDelete }) {
  /* [v23] 유형은 개인레슨 · 듀엣 · 그룹 3가지. 회원 선택은 개인·듀엣만, 그룹은 기구 선택 */
...
/* 각도: atan2(dy, dx) × 180/π. 왼쪽 점이 앞에 오도록 정렬 → -90°~+90°
   deg > 0 이면 오른쪽 끝이 아래로 내려간 것 = 화면 좌측이 더 높음 */
...
function PostureCanvas({ photo, label, onClose, onSave, onToast }) {
  ...
  const down = (e) => {
    ...
    const p = pos(e);
    /* [v24] 수평선·수직선도 누르고 있는 동안 위치를 잡고, 손을 뗄 때 확정합니다 */
...
function MemberList({ members, selectedId, onSelect, onAdd, onOpenFav, favCount, schedule }) {
  ...
  const [seg, setSeg] = useState("active"); // active | hold | ended
  const [todayOnly, setTodayOnly] = useState(false);   // [v15] 오늘 수업 회원만 보기
  const [sortBy, setSortBy] = useState("default");     // [v25] default | leftDesc | leftAsc
  /* 오늘 스케줄에 있는 회원 = 오늘 관리해야 할 회원 */
  ...
  const filtered = members
    ...
    .sort((a, b) => {
      ...
      if (ta && tb) return ta.localeCompare(tb);      // 오늘 수업 회원은 시간순으로 위에
      if (ta) return -1;
      if (tb) return 1;
      return 0;
    });
  return (
    <div className="space-y-3">
      ...
      {/* [v25] 잔여 횟수 정렬 — 누를 때마다 많은 순 → 적은 순 → 기본 순으로 바뀝니다 */}
      {filtered.length > 1 && (
      ...
      {filtered.map((m) => {
        ...
        const lastD = lastDoneOf(schedule, m.id);      // [v10] 실제 수업 이력 기준
        const idle = lastD ? Math.max(0, -dday(lastD)) : null;
...
function PhotoCompare({ member, photos, briefing, onSavePhoto, onRemove, onSaveMarks, onAdjust, onToast }) {
  ...
  const [adjust, setAdjust] = useState(null);   /* [v24] 이미 등록한 사진 다시 맞추기 */
  ...
  /* [v27] 촬영하기 한 줄만 — 사진이 없는 칸에는 위 안내 상자에 촬영·불러오기가 이미 있어 중복을 없앴습니다 */
  const ShotBar = ({ slot }) => (
        ...
        <div className="mt-4 flex gap-3">
          {[{ p: before, label: "BEFORE", slot: "before" }, { p: after, label: "AFTER", slot: "after" }].map((s) => (
            <div key={s.slot} className="min-w-0 flex-1">
              ...
              {/* [v27] 사진이 있는 칸에만 촬영하기 — 빈 칸은 위 상자에서 촬영·불러오기 */}
...
/* ==================================================================
   [v9] 종합 평가
   - 비교할 인바디 측정일을 직접 선택
   - 인바디 + 운동 수행 능력 + 출석률/페이스를 함께 분석
   - 회원용 멘트 / 강사용 멘트를 따로 생성
================================================================== */
/* [v27] AI 응답이 없을 때 쓰는 오프라인 멘트 — 화면과 같은 데이터로 문장을 만듭니다 */
...
function OverallReview({ member, briefing, onToast, schedule }) {
  ...
  /* [v22] 같은 날을 고르면 반대쪽 날짜가 자동으로 비켜 준다 */
  ...
  /* [v27] 인터넷·AI 응답이 없어도 화면이 비지 않게 — 같은 데이터로 문장을 직접 만듭니다 */
  ...
  const make = async (kind) => {
    ...
    try {
      ...
      /* [v27] 실패해도 빈 화면을 주지 않습니다 — 지금 데이터로 바로 만들어 보여 줍니다 */
...
/* ==================================================================
   [v6 신규] 비포 & 애프터 세트
   - 사진이 2장 이상 되면 (비포, 애프터) 한 쌍이 자동으로 세트 목록에 저장됨
   - 세트마다 즐겨찾기(★) 지정 → 회원 목록 상단 '즐겨찾기' 버튼에서 모아 보기
================================================================== */
function SetThumb({ p }) {
  return (
...
function SetViewer({ item, onClose, onToggleFav }) {
  ...
  if (!before || !after || !set) return null; // [v18] 짝이 깨진 사진 세트 방어
...
/* ==================================================================
   [v20] AI 체형 분석 — PoseAnalyzer
   ① 강사가 회원의 정면/측면 전신사진을 업로드
   ② MediaPipe PoseLandmarker(33점)로 어깨·골반·무릎·발목 등 관절 자동 인식
   ③ 관절을 점과 선으로 캔버스에 그리고, 삼각함수로 각도를 계산해
      "어깨 틀어짐 4.5°" 처럼 사진 위에 바로 표시
   ④ 모델을 내려받지 못하는 환경(사내망·미리보기)에서는 관절을 직접 찍는
      수동 모드로 동일한 계산을 수행 — 어떤 환경에서도 기능이 죽지 않는다
================================================================== */
...
function loadLandmarker() {
  ...
  mpPromise = (async () => {
    /* 번들러가 URL import를 가로채지 못하도록 우회 — 실패하면 일반 dynamic import */
    let dyn = null;
    try { dyn = new Function("u", "return import(u)"); } catch (e) { dyn = null; }
    let vision = null, wasmBase = MP_WASM[0];
    for (let i = 0; i < MP_LIB.length; i++) {
      try {
        const mod = dyn ? await dyn(MP_LIB[i]) : await import(/* @vite-ignore */ /* webpackIgnore: true */ MP_LIB[i]);
...
/* 수동 지정 안내 — 좌/우는 항상 '회원 기준'으로 받고, 화면에서 어느 쪽인지 함께 알려준다
   (셀카처럼 좌우가 뒤집힌 사진은 '좌우 바꿈' 버튼으로 정정) */
...
/* 수동 모드에서 찍는 순서 — 화면 기준으로 안내하고 내부에서 좌우를 뒤집는다 */
...
const lvDown = (v, ok, warn) => (v <= ok ? "good" : v <= warn ? "warn" : "bad"); // 작을수록 좋음
const lvUp = (v, ok, warn) => (v >= ok ? "good" : v >= warn ? "warn" : "bad");   // 클수록 좋음
const LV_RANK = { good: 0, warn: 1, bad: 2 };

/* ---------- 분석 본체 ---------- */
/* pts: { 이름: {x,y,score} } — 0~1 정규화 좌표(사진 기준)
   각도는 반드시 실제 픽셀 좌표로 계산해야 사진 비율에 따라 틀어지지 않는다 */
function analyzePose(raw, { view, W, H, floorFix }) {
  ...
  if (view === "front") {
    ...
    /* 촬영 기울기 — 발목선(바닥선)이 기울면 사진 자체가 기운 것일 수 있다 */
    ...
    items.push({
      key: "shoulder", label: "어깨 틀어짐 각도", value: Math.abs(r1(sh)), unit: "°",
      level: lvDown(Math.abs(sh), 2, 4), at: midOf(P.shL, P.shR), up: -1,
      dir: Math.abs(sh) < 0.3 ? "수평" : `${shHigh} 높음`,
      desc: Math.abs(sh) < 0.3 ? "좌우 어깨 높이가 거의 같습니다." : `${shHigh} 어깨가 ${Math.abs(r1(sh))}° 올라가 있습니다.`,
      tip: `${shHigh} 상부승모근·견갑거근 이완 → 반대쪽 하부승모근·전거근 강화`,
    });
    items.push({
      key: "pelvis", label: "골반 비대칭 각도", value: Math.abs(r1(hip)), unit: "°",
      level: lvDown(Math.abs(hip), 2, 4), at: midOf(P.hipL, P.hipR), up: 1,
      dir: Math.abs(hip) < 0.3 ? "수평" : `${hipHigh} 높음`,
      desc: Math.abs(hip) < 0.3 ? "좌우 골반 높이가 거의 같습니다." : `${hipHigh} 골반이 ${Math.abs(r1(hip))}° 올라가 있습니다.`,
      tip: `${hipHigh} 요방형근 이완 → 반대쪽 중둔근 활성 (사이드 라잉 시리즈)`,
    });
    const twist = Math.abs(sh - hip);
    items.push({
      key: "twist", label: "어깨-골반 기울기 차", value: r1(twist), unit: "°",
      level: lvDown(twist, 2, 5), at: midOf(midOf(P.shL, P.shR), midOf(P.hipL, P.hipR)), up: 0, skipBadge: true,
      dir: twist < 1 ? "정렬 양호" : sh * hip < 0 ? "반대로 기움" : "같은 방향",
      desc: `어깨선과 골반선의 기울기가 ${r1(twist)}° 차이 납니다.${sh * hip < 0 ? " 두 선이 서로 반대로 기운 형태로, 측만 성향을 함께 확인해 보세요." : ""}`,
      tip: "짧아진 쪽 측면 라인(요방형근·광배근) 이완 후 반대쪽 측면 강화 · 사이드 벤드 좌우 비교",
    });

    /* 무릎 — 관절각만 보면 정상 다리(180°)가 전부 과신전으로 잡힌다.
       고관절-발목 직선 대비 무릎이 안/밖으로 얼마나 밀렸는지로 판단한다 */
    if (has("kneeL", "kneeR", "ankL", "ankR")) {
      ...
      const legs = ["L", "R"].map((s) => {
        ...
        const inward = (midX - kneeP.x) * dx > 0; // 중심선 쪽으로 밀렸는가
      ...
      items.push({
        key: "knee", label: "무릎 정렬 각도", value: worst.dev, unit: "°",
        level: lvDown(worst.dev, 3, 6), at: worst.at, up: -1,
        dir: shape,
        desc: `왼쪽 ${legs[0].dev}° · 오른쪽 ${legs[1].dev}° — ${shape}`,
        tip: worst.dev < 3 ? "현재 정렬 유지 · 스쿼트 시 무릎-2번 발가락 정렬 큐잉"
          : worst.inward ? "중둔근·고관절 외회전근 강화, 발 아치 지지" : "내전근·족부 회내 컨트롤, 스쿼트 무릎 궤적 교정",
      });
    }
    if (has("earL", "earR")) {
      ...
      items.push({
        key: "head", label: "머리 기울기", value: Math.abs(r1(head)), unit: "°",
        level: lvDown(Math.abs(head), 2, 4), at: midOf(P.earL, P.earR), up: -1,
        dir: Math.abs(head) < 0.3 ? "수평" : `${hh} 기움`,
        desc: `귀 높이가 ${Math.abs(r1(head))}° 차이 납니다.`,
        tip: `${hh} 사각근·흉쇄유돌근 이완 · 턱 당기기(chin tuck) 병행`,
      });
    }
    /* 좌우 무게중심 — 각도가 아니라 어깨너비 대비 %.
       좌/우 판정은 화면이 아니라 회원의 실제 좌우(어깨 좌표 순서)를 따른다 */
    if (has("ankL", "ankR")) {
      const shW = Math.abs(P.shL.x - P.shR.x) || 1;
      /* 보정을 켜면 '수평'이 아니라 '바닥선 방향'으로 재어야 사진 기울기가 값에 섞이지 않는다 */
      ...
      const dirL = Math.sign(P.shL.x - P.shR.x) || 1;         // 회원의 왼쪽이 화면에서 어느 방향인지
  ...
  /* 거북목 — 임상 CVA는 C7 기준이라 사진의 어깨(견봉)로는 그대로 쓸 수 없다.
     대신 '귀가 어깨 수직선에서 얼마나 앞으로 나갔는가'(전방 두부 각)로 계산한다.
     귀-어깨-골반-복사뼈가 한 수직선에 놓인 상태가 0°. */
  const fha = vertDeg(sh, ear);
  items.push({
    key: "fha", label: "거북목(머리 전방 이동)", value: Math.abs(r1(fha)), unit: "°",
    level: lvDown(Math.abs(fha), 7, 15), at: midOf(ear, sh), up: -1,
    dir: Math.abs(fha) < 3 ? "정렬 양호" : `머리가 ${fw(fha)}`,
    desc: `귀가 어깨 수직선보다 ${Math.abs(r1(fha))}° ${fw(fha)}에 있습니다. 7° 이내면 양호, 15°를 넘으면 전방 두부 자세로 봅니다.`,
    tip: "턱 당기기(chin tuck) 10회 × 3 · 흉추 신전 가동 · 모니터/베개 높이 점검",
  });
  const lean = vertDeg(hip, sh);
  items.push({
    key: "trunk", label: "몸통 기울기", value: Math.abs(r1(lean)), unit: "°",
    level: lvDown(Math.abs(lean), 3, 6), at: midOf(hip, sh), up: 0,
    dir: Math.abs(lean) < 1 ? "수직 정렬" : `${fw(lean)}으로 기움`,
    desc: `골반-어깨 선이 수직에서 ${Math.abs(r1(lean))}° 벗어나 있습니다.`,
    tip: "복부 심부근 활성 + 흉요추 분절 컨트롤 (footwork·bridging 큐잉)",
  });
  if (knee && ank) {
    const dev = bendDeg(hip, knee, ank);
    const dx = xDev(hip, ank, knee);
    const back = dx * face < 0; // 무릎이 뒤로 밀림 = 과신전
    items.push({
      key: "kneeSide", label: back ? "무릎 과신전(반장슬)" : "무릎 굴곡", value: r1(dev), unit: "°",
      level: lvDown(dev, 3, 6), at: knee, up: -1,
      dir: dev < 2 ? "중립" : back ? "뒤로 밀림" : "앞으로 굽음",
      desc: `고관절-발목 선 대비 무릎이 ${r1(dev)}° ${back ? "뒤" : "앞"}으로 벗어나 있습니다.`,
      tip: back ? "무릎 살짝 풀고 서기(soft knee) · 햄스트링·종아리 이완, 대퇴사두 편심 강화"
        : "고관절 신전 가동 확보 · 장요근 이완 후 둔근 활성",
    });
    const glob = vertDeg(ank, ear);
    items.push({
      key: "align", label: "전신 수직 정렬", value: Math.abs(r1(glob)), unit: "°",
      level: lvDown(Math.abs(glob), 4, 8), at: ank, up: -1, skipBadge: true,
      dir: Math.abs(glob) < 1 ? "정렬 양호" : `머리가 ${fw(glob)}`,
      desc: `복사뼈에서 올린 수직선 대비 귀가 ${Math.abs(r1(glob))}° ${fw(glob)}에 있습니다.`,
      tip: "발-골반-흉곽-머리 스택 재정렬 (벽 서기 30초 × 3)",
    });
  }
...
function PoseAnalyzer({ member, photos, onSavePose, onDeletePose, onToast }) {
  ...
  /* 라이브러리 준비 — 카드를 펼칠 때 한 번만 시도 */
  useEffect(() => {
    if (!open || engine !== "idle") return;
    let alive = true;
    setEngine("loading");
    loadLandmarker()
      .then(() => { if (alive) setEngine("ready"); })
      .catch(() => { if (alive) setEngine("manual"); });
    return () => { alive = false; };
  }, [open, engine]);
  ...
  const draw = useCallback(() => {
    ...
    if (showSkel) {
      ...
      /* 관절 핸들 — 헤일로 + 링 + 코어. 잡고 있는 점은 커지고 이름표가 붙는다 */
      Object.keys(pts).forEach((key) => {
        ...
        if (on) {
          ctx.strokeStyle = "rgba(255,255,255,.55)"; ctx.lineWidth = 1.4 * k;
        ...
        ctx.beginPath(); ctx.arc(p.x, p.y, r * 0.46, 0, Math.PI * 2); ctx.fill();
        ctx.restore();
      });
      /* 잡는 중 — 십자선 · 확대경 · 이름표 */
      if (hot && pts[hot]) {
    ...
    if (showNum && res) {
      ...
      res.items.filter((i) => !i.skipBadge).forEach((i) => {
        badge(ctx, i.at.x * (c.width / img.w), i.at.y * (c.height / img.h),
          `${i.label.replace(/\(.*\)/, "").trim()} ${i.value}${i.unit}`, LV_COLOR[i.level], i.up ?? -1, placed, c.width, c.height);
      });
    }
  ...
  const detect = async (im) => {
    try {
      ...
      /* 정면·측면 자동 판별 — 어깨 너비가 몸통 길이의 35% 미만이면 측면 */
  ...
  const onDown = (e) => {
    ...
    if (manual && manual.i < manual.seq.length) {
      ...
      dragRef.current = key; setHot(key); buzz(6);   // 찍은 뒤 바로 미세 조정 가능
      e.currentTarget.setPointerCapture?.(e.pointerId);
    }
  };
  const onMove = (e) => {
...
function RecordTab({ db, selectedId, setSelectedId, section, setSection, onSaveInbody, onDeleteInbody, onSaveNote, onPatch, onDelete, onToast, onSettings }) {
  const [openInfo, setOpenInfo] = useState(false);   /* [v27] 회원 정보 접기 */
  ...
  return (
    <div className="space-y-3">
      <Card className="p-4">
        ...
        {/* [v27] 회원 정보 간략히 — 화살표를 누르면 자세히, 다시 누르면 접힙니다 */}
...
/* ==================================================================
   [v27] 인바디 결과지 QR — 기기·전송 설정에 따라 내용이 달라서 네 가지를 모두 받아들입니다
   ① JSON  ② key=value 나열  ③ 링크의 물음표 뒤 값  ④ '체중 70.2kg' 처럼 한글 라벨이 붙은 글
   값이 없는 링크(인바디 결과 페이지로 이동만 하는 QR)면 값을 채우지 않고 그대로 알려 줍니다
================================================================== */
const QR_FIELDS = [
...
function parseInbodyQR(raw) {
  ...
  QR_FIELDS.forEach((f) => f.keys.forEach((k) => {   /* ④ '체중 70.2kg' · 'BMI 27.1' 처럼 띄어 쓴 라벨 */
  ...
  pairs.forEach((kv) => {
    ...
    if (n < f.min || n > f.max) return;                 /* 범위를 벗어난 값은 넣지 않습니다 */
    out.values[f.k] = n;
  });
...
/* [v27] QR 읽기 화면 — 카메라 → 사진 → 붙여넣기 순으로 준비된 방법을 내어 줍니다 */
...
function InbodyForm({ member, last, onSave, onDelete, onPatch, onToast }) {
  ...
  const [qr, setQr] = useState(false);          /* [v27] QR 읽기 시트 */
  const [qrHit, setQrHit] = useState(null);     /* 방금 QR로 채운 항목 */
    ...
    <>
      ...
      <Card className="p-5">
        <div className="flex items-center gap-2">
          ...
          {/* [v27] 결과지 QR을 찍으면 입력칸이 채워집니다 (저장은 확인 후 직접) */}
        ...
        <div className="mt-4 space-y-3">
          ...
          <PrimaryBtn disabled={!ready} onClick={() => {
            onSave(member.id, {
              ...
              fatMass: num(f.fatMass), bmi: num(f.bmi), bmr: num(f.bmr), visceral: num(f.visceral), score: num(f.score),
  ...
  return (
    <Card className="p-5">
      ...
      <div className="mt-4 space-y-3">
        ...
        {/* [v17] 자주 쓰는 문구 — 탭하면 아래 내용에 이어 붙습니다 */}
        <div className="rounded-2xl p-3" style={{ backgroundColor: CANVAS }}>
          <div className="flex items-center justify-between">
...
/* [v27] 수행 능력 점수는 어떤 값이 들어와도 0~100 정수로 맞춥니다 */
const clampScore = (v) => Math.max(0, Math.min(100, Math.round(Number(v) || 0)));

function PerfForm({ member, onPatch }) {
  const [newName, setNewName] = useState("");
  const rows = Array.isArray(member.perf) ? member.perf : [];   /* [v27] perf 가 비어 있어도 안전하게 */
        ...
        {rows.map((p, i) => {
          ...
          return (
            <div key={i} className="rounded-2xl p-3" style={{ backgroundColor: CANVAS }}>
              ...
              {/* [v27] 1점씩 조절 — 왼쪽 끝 −, 오른쪽 끝 +. 슬라이더는 min-w-0 이라 칸을 벗어나지 않습니다 */}
...
/* ==================================================================
   [v7] 회원 정보 — 임시 편집 후 '저장하기'로 확정
        수강권은 결제(등록) 이력으로 관리 → 누적 결제액 자동 집계
================================================================== */
...
function InfoForm({ member, onPatch, onDelete, onToast }) {
  ...
  const [hist, setHist] = useState(false); // [v8] 결제 내역 시트
...
/* ==================================================================
   [v27] 회원 인계 — 고유번호(인계 코드)로 다른 강사에게 DB를 넘깁니다
   · 헷갈리는 글자(0·O·1·I)는 코드에서 뺐습니다
   · 같은 앱을 쓰는 기기끼리는 번호만으로, 그 밖에는 인계 코드 텍스트를 붙여넣어 받습니다
   · 인계 코드는 비밀번호와 같으니 받는 분에게만 알려 주세요
================================================================== */
...
/* 받은 자료 합치기 — 같은 회원 번호는 받은 쪽으로 갱신, 없던 회원은 새로 추가 */
...
function HandoffCard({ db, photos, account, onImport, onToast }) {
  const [mode, setMode] = useState(null);          /* null · give · take */
  ...
  const make = async () => {
    ...
    if (withPhotos && text.length > 4.5 * 1048576) {           /* 저장 한도를 넘으면 사진은 빼고 만듭니다 */
...
function SettingsTab({ db, photos, account, onChangeSettings, onChangePhoto, savedAt, onReset, onClear, onLogout, onToast, themePref, onChangeTheme, onImport }) {
  ...
  const [busy, setBusy] = useState(false);
  const camRef = useRef(null), albumRef = useRef(null);
  /* [v20] 프로필 사진 — 320px 정사각 썸네일로 줄여 계정에 저장 */
...
export default function App() {
  ...
  const [tab, setTab] = useState("schedule");   // [v15] 앱을 열면 오늘 스케줄부터
  const [section, setSection] = useState("inbody");
  const [selectedId, setSelectedId] = useState(null);
  const [mobileView, setMobileView] = useState("list");
  const briefing = false; // [v18] 헤더 브리핑 토글 제거 — 항상 전체 정보 표시
  const [savedAt, setSavedAt] = useState(null);
  const [toast, setToast] = useState(null);
  const [brief, setBrief] = useState(null);
  const [favOpen, setFavOpen] = useState(false); // [v6] 즐겨찾기 비포&애프터 모아보기
  /* [v21] 화면 테마 — 기본값은 폰 설정(system)을 따라간다 */
  ...
  /* [v20] 프로필 사진 저장 — 계정 목록과 현재 세션 계정에 동시 반영 */
  ...
  const saveSchedule = (item) => {
    ...
    if (prev) {                                     /* [v23] 수정으로 빠진 회원의 차감은 복구 */
  ...
  /* [v9] 회원별 출석 처리 — 그룹 수업은 attendees 중 해당 회원만 변경 */
  ...
  /* [v23] 기구 그룹 수업 — 회원 차감 대신 수업 단위로 완료 처리 */
  ...
  const saveNote = (id, note) => {
    const t = db.members.find((m) => m.id === id);
    patch(id, { notes: [note, ...(t.notes || [])] });   // [v14] 코멘트는 차감하지 않음
  ...
  const savePhoto = (view, src, slot, tf) => {
    ...
    // [v6] 비포·애프터가 한 쌍이 되면 세트 목록에 자동 저장
    const sets = [...(cur.sets || [])];
    let made = false;
  ...
  const style = (
    <style>{`
      ...
      /* [v21] 다크 모드 — Tailwind 기본 색 클래스를 테마 값으로 덮어쓴다 */
      ...
      /* [v19] 한글은 어절 단위로만 줄바꿈 — 모든 기기에서 글자 배열 유지 */
  ...
  return (
    <div className="app-root min-h-screen pb-16" style={{ backgroundColor: PAGE }}>
      {style}
      {/* [v24] 헤더와 탭을 하나의 고정 영역으로 — 기기 폭에 상관없이 겹치지 않습니다 */}
